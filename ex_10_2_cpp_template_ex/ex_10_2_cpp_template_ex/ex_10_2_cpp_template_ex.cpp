/*
We saw how overloading 
can be used to create a function 
that is fairly generic. 
Unfortunately, we still have to write 
a function for each variable type. 
Wouldn't it be nice to just write 
a function once and use it over 
and over for different variable types?
Well, actually, we can do that.
C++ allows us to create generic functions using templates.
Let's look at an example of a function built using a template. 
Then we'll discuss the details of what is a template 
and how to use them to create generic functions.
